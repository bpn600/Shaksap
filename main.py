...
class MainScreen(Screen):
    ...
    def share_facebook(self):
        msg = urllib.parse.quote(self.text_input.text or "")
    ...
    # Safe sound loading for both platforms
    def ctrl_sound(self):
        try:
            sound_path = os.path.join(os.path.dirname(__file__), 'assets', 'sound', 'clickbtn.mp3')
            self.sound = SoundLoader.load(sound_path)
        except Exception as e:
            print(f"Sound not loaded: {e}")
            self.sound = None
